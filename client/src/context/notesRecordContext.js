...
export const NotesRecordProvider = ({children}) => {
    ...
    const fetchRecord = async () => {
        if (!user) return;
        const response = await fetch(`https://ai-notesapp-server.onrender.com/api/notes/${user.id}`)
    ...
    const addRecord = async (record) => {
        const response = await fetch('https://ai-notesapp-server.onrender.com/api/notes',{
            method: 'POST',
    ...
    const updateRecord = async (id,newRecord) => {
        const response = await fetch(`https://ai-notesapp-server.onrender.com/api/notes/${id}`,{
            method: 'PUT',
